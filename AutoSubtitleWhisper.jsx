...
/*  Whisper Auto Subtitles (ScriptUI Panel) - Windows/Mac
    Requirements:
      - Place in: Scripts/ScriptUI Panels/AutoSubtitleWhisper.jsx
...
// No lookbehind (ExtendScript limitation). Split large blocks on punctuation conservatively.
function splitTranscriptToLines(txt) {
    ...
    for (var b = 0; b < blocks.length; b++) {
        ...
        if (block.length > 140) {
            // Insert newlines after sentence punctuation, then split
            var rebuilt = "";
            for (var i = 0; i < block.length; i++) {
                var ch = block.charAt(i);
                rebuilt += ch;
                if ((ch === '.' || ch === '!' || ch === '?') && i < block.length - 1) {
                    var next = block.charAt(i + 1);
                    if (next === ' ') rebuilt += '\n';
                }
            }
            var segs = rebuilt.split(/\n+/);
    ...
    for (var k = 0; k < lines.length; k++) {
        ...
        for (var w = 0; w < words.length; w++) {
            ...
            if (tryAdd.length > 90) { if (curr) wrapped.push(curr); curr = words[w]; }
            else curr = tryAdd;
        }
...
function buildWhisperCommand(mediaPath, outDir, model, language, fmt) {
    ...
    if (isWindows()) {
        return 'cmd /c ' + cmdCore; // IMPORTANT: do NOT quote the entire command
    } else {
...
function showEditor() {
    ...
    function collectTextLayersBottomToTop() {
        ...
        for (var i = 1; i <= comp.numLayers; i++) {
            var L = comp.layer(i);
            if (L instanceof TextLayer) arr.unshift(L);
        }
        return arr;
    }
...
function buildUI(panel) {
	...
	modelDD.alignment = ["fill", "center"];
    row1.add("statictext", undefined, "Language:");
    var langET = row1.add("edittext", undefined, "English"); langET.characters = 12;
	langET.alignment = ["fill", "center"];

    var row2 = whisperPanel.add("group"); row2.orientation = "row";
    row2.add("statictext", undefined, "Output Folder:");
    var outET = row2.add("edittext", undefined, ""); outET.characters = 34; outET.alignment = ["fill", "center"];
    var outBtn = row2.add("button", undefined, "Browse"); outBtn.alignment = ["fill", "center"];

    var row3 = whisperPanel.add("group"); row3.orientation = "row"; row3.alignChildren = ["fill","center"];
    var transcribeBtn = row3.add("button", undefined, "Generate Subtitles"); 
    var refreshSelBtn = row3.add("button", undefined, "Use Selected Layer");
    var statusST = whisperPanel.add("statictext", undefined, "Status: Idle"); statusST.characters = 60;
    ...
    socialGroup.orientation = "row"; socialGroup.spacing = 10; socialGroup.alignChildren = ["fill","center"];
    ...
    function runWhisperAndImport() {
        ...
        if (!srtFile.exists) {
            ...
            if (txtContent && txtContent.replace(/\s+/g, '').length > 0) {
                ...
                if (!ok || !srtFile.exists) { alert("Whisper did not create .srt, and failed to build SRT from .txt."); statusST.text = "Status: No SRT created."; return; }
                statusST.text = "Built SRT from TXT (auto-timed).";
            } else if (sysOut && sysOut.replace(/\s+/g, '').length > 0) {
                var totalDur2 = Math.max(ly.outPoint - ly.inPoint, comp.duration);
                var ok2 = makeSRTFromTXT(sysOut, srtFile, totalDur2);
                if (!ok2 || !srtFile.exists) { alert("No .srt found. Check Whisper CLI or set output folder.\n\nCommand:\n" + cmd); statusST.text = "Status: No SRT created."; return; }
...
// ---------------------------
// SINGLE-PANEL ENTRY (matches your importer pattern)
// Name this variable exactly like the file: AutoSubtitleWhisper.jsx
// ---------------------------
var AutoSubtitleWhisper = (this instanceof Panel) ? this : new Window("palette", "Whisper Auto Subtitles", undefined, {resizeable:true});
buildUI(AutoSubtitleWhisper);

// If run manually (not loaded as panel), show as floating palette
if (AutoSubtitleWhisper instanceof Window) {
    AutoSubtitleWhisper.center();
    AutoSubtitleWhisper.show();
}
